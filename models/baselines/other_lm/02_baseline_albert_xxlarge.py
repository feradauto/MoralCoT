...
def assign_prompt(df,prompt_number=0):
    if prompt_number==0:
        df=df.assign(pre_statement=np.where(df.study=='blue.house',"Analyze Hank's action. Is it acceptable to break the rule in this situation? Answer \"Yes\" or \"No\".","Analyze that person's action. Is it acceptable to break the rule in this situation? Answer \"Yes\" or \"No\".")) 
    elif prompt_number==1:
        df=df.assign(pre_statement=np.where(df.study=='blue.house',"Analyze Hank's action. Is it ok to break the rule in this situation? Answer \"Yes\" or \"No\".","Analyze that person's action. Is it ok to break the rule in this situation? Answer \"Yes\" or \"No\"."))
    elif prompt_number==2:
        df=df.assign(pre_statement=np.where(df.study=='blue.house',"Is it ok to break the rule in this situation? Answer \"Yes\" or \"No\".","Is it ok to break the rule in this situation? Answer \"Yes\" or \"No\"."))
    else:
         df=df.assign(pre_statement=np.where(df.study=='blue.house',"Analyze Hank's action. Is it acceptable to break the rule in this situation? Answer \"Yes\" or \"No\".","Analyze that person's action. Is it acceptable to break the rule in this situation? Answer \"Yes\" or \"No\".")) 
...
def get_prediction(filename,prompt_number):
   ...
   """
    ...
    model = pipeline('fill-mask', model='albert-xxlarge-v1',top_k=15)
...
def main():
    ...
    parser.add_argument('-o','--output_file', nargs='?', help='output file name',default='./albert_predictions.csv')
